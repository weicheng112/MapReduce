...
// It uses random selection to distribute chunks evenly across nodes
func (c *Controller) selectStorageNodes(chunkSize int) []string {
	...
	// Randomly shuffle the available nodes for even distribution
	rand.Shuffle(len(availableNodeIDs), func(i, j int) {
		availableNodeIDs[i], availableNodeIDs[j] = availableNodeIDs[j], availableNodeIDs[i]
	})
